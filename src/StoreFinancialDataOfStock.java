...
public class StoreFinancialDataOfStock extends SetupBase {
	...
	public static void main(String[] args) {
		...
		tmpStoreFinancialDataOfStock.getFiancialDataForStock("533320");
	...
	private void StoreFinancialDataIndicatorToDB(StockFinancialIndicator stockFinancialIndicatortmp) {
		...
		try {
			...
			connection = DriverManager.getConnection(CONNECTION_STRING, USER, PASS);
			statement = connection.createStatement();
			tmpsql = "INSERT INTO STOCK_FINANCIAL_TRACKING (STOCKNAME, BSECODE, DATECOLLECTED, ANNUALSALESINDICATOR, QUARTERSALESINDICATOR) VALUES('" + 
    				stockName + "','" + bseCode + "','" + new Date() + "','" + stockFinancialIndicatortmp.annualSalesIndicator + "','" + stockFinancialIndicatortmp.quarterSalesIndicator + "');";
			statement.executeUpdate(tmpsql);
			connection.close();
			connection = null;			
		} catch (Exception ex) {
