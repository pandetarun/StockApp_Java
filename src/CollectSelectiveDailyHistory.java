...
public class CollectSelectiveDailyHistory extends SetupBase {
	...
	public static void main(String[] args) {
		...
		CollectSelectiveDailyHistory obj = new CollectSelectiveDailyHistory();
	...
	public void startCollectingHistoryData() {
		...
		ArrayList<String> unProcessedStockList;

		try {
			unProcessedStockList = unProcessedStocks();
			setupSelenium(URL);
			...
			for (String unProcessedStock : unProcessedStockList) {
				FileUtils.cleanDirectory(new File(downloadFilepath));
				getHistoryData(unProcessedStock);
	...
	private void readFileAndPopulateStockQuote(String enteredStockLastDate) {
		...
		try {
			...
			for (File inputFile : inputFileList) {
				if (inputFile.isFile()) {
					...
					while (line != null) {
						...
						// System.out.println("Line read at " + i++ + " Date ->
						// "+stockData[2]);
						if ((stockData[1].substring(1, stockData[1].length() - 1).trim().equalsIgnoreCase("EQ")
								|| stockData[1].substring(1, stockData[1].length() - 1).trim().equalsIgnoreCase("BE"))
								&& enteredStockLastDate == null) {
							quotesDataObj.stockName = Parser.unescapeEntities(
									stockData[0].substring(1, stockData[0].length() - 1).trim(), false);
							quotesDataObj.quoteDate = stockData[2].substring(1, stockData[2].length() - 1).trim();
							quotesDataObj.openPrice = Float
									.parseFloat(stockData[4].substring(1, stockData[4].length() - 1).trim());
							quotesDataObj.dailyHigh = Float
									.parseFloat(stockData[5].substring(1, stockData[5].length() - 1).trim());
							quotesDataObj.dailyLow = Float
									.parseFloat(stockData[6].substring(1, stockData[6].length() - 1).trim());
							quotesDataObj.closingPrice = Float
									.parseFloat(stockData[8].substring(1, stockData[8].length() - 1).trim());
							quotesDataObj.volume = Long
									.parseLong(stockData[10].substring(1, stockData[10].length() - 1).trim());
							...
							if (enteredStockLastDate != null
									&& eneterdDate
											.getTime() < Date
													.parse(stockData[2].substring(1, stockData[2].length() - 1)
															.trim())
									&& (stockData[1].substring(1, stockData[1].length() - 1).trim()
											.equalsIgnoreCase("EQ")
											|| stockData[1].substring(1, stockData[1].length() - 1).trim()
													.equalsIgnoreCase("BE"))) {
					...
					Files.move(Paths.get(inputFile.getAbsolutePath()),
							Paths.get("C:/Selenium/downlodProcessed/" + inputFile.getName()));
					// inputFile.renameTo(new
					// File("C:/Selenium/downlodProcessed"));
				}
			}
			...
			System.out.println("readFileAndPopulateStockQuote for quote -> " + quotesDataObj.stockName
					+ "  and Date -> " + quotesDataObj.quoteDate + " Error -> " + ex.getMessage());
			enterhandledStock();
			try {
				Files.move(Paths.get(inputFileForException.getAbsolutePath()),
						Paths.get("C:/Selenium/downlodProcessed/" + inputFileForException.getName()));
			} catch (Exception exx) {
	...
	private void storeQuotestoDB() {
		...
		try {
			...
			tmpsql = "INSERT INTO DAILYSTOCKDATA (STOCKNAME, CLOSEPRICE, HIGHPRICE, LOWPRICE, OpenPrice, VOLUME, TRADEDDATE) VALUES('"
					...
					+ quotesDataObj.dailyLow + "," + quotesDataObj.openPrice + "," + quotesDataObj.volume + ",'"
					+ quotesDataObj.quoteDate + "');";
			statement.executeUpdate(tmpsql);
		} catch (Exception ex) {
			System.out.println("storeQuotestoDB for quote -> " + quotesDataObj.stockName + " and Date - > "
					+ quotesDataObj.quoteDate + " Error in DB action" + ex);
		}
	}
	...
	private void enterhandledStock() {
		...
		try {
			statement = connection.createStatement();
			tmpsql = "INSERT INTO ENTEREDSTOCKDETAILS (STOCKNAME, LASTDATE) VALUES('" + quotesDataObj.stockName + "','"
					+ quotesDataObj.quoteDate + "');";
			statement.executeUpdate(tmpsql);
		} catch (Exception ex) {
			System.out.println("enterhandledStock for quote -> " + quotesDataObj.stockName + " and Date - > "
					+ quotesDataObj.quoteDate + " Error in DB action" + ex);
		}
	}

	private ArrayList<String> unProcessedStocks() {
		BufferedReader br = null;
		ArrayList<String> stocklistFromFile = new ArrayList<String>();
		try {
			br = new BufferedReader(new FileReader("C:/Selenium/stocklist.txt"));
			String line = br.readLine();
			while (line != null) {
				stocklistFromFile.add(line);
				line = br.readLine();
	...
	private String getEneteredStockData(String stockCode) {
		...
		try {
			...
			tmpsql = "select LASTDATE from ENTEREDSTOCKDETAILS where STOCKNAME = '" + stockCode
					+ "' and LASTDATE < '5-Jun-2017';";

			// tmpsql = "SELECT First 100 sd.NSECODE FROM STOCKDETAILS sd LEFT
			// JOIN ENTEREDSTOCKDETAILS esd ON sd.NSECODE = esd.STOCKNAME WHERE
			// esd.STOCKNAME IS NULL Or esd.LASTDATE < '5-Jun-2017';";
